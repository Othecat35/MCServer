...
# Types
dependency_types: tuple[str, str, str, str] = (
    "embedded",
    "optional",
    "required",
    "incompatible",
...
def create(
    ...
    for dependency_id, dependency_type in dependencies.items():
        if dependency_type not in dependency_types:
            log.error(
                f"Invalid dependency type '{dependency_type}' for dependency ID '{dependency_id}' of project ID '{project_id}'"
            )
            continue
    ...
    for dependent_id, dependent_type in dependents.items():
        if dependent_type not in dependency_types:
            log.error(
                f"Invalid dependency type '{dependent_type}' for dependent ID '{dependent_id}' of project ID '{project_id}'"
            )
            continue
...
def read(project_id: str) -> ProjectRelationships:
    ...
    try:
        ...
        log.error(
            f"Error parsing relationships file for project ID '{project_id}': {error.msg} at line {error.colno} column {error.colno}"
        )
        return project_relationships
    ...
    for dependency_id, dependency_type in dependencies.items():
        if dependency_type not in dependency_types:
            log.error(
                f"Invalid type '{dependency_type}' for dependency ID '{dependency_id}' of project ID '{project_id}'"
            )
            continue
    ...
    for dependent_id, dependent_type in dependents.items():
        if dependent_type not in dependency_types:
            log.error(
                f"Invalid type '{dependent_type}' for dependent ID '{dependent_id}' of project ID '{project_id}'"
            )
            continue
